...
(function configureEvergage() {
    try {
       Evergage.configure({
           ...
           trackerUrl: "https://fossil.us-1.evergage.com",
           siteConfigVersion: "57"
       },{"allowBotTraffic":false,"corsAllowedOrigins":["*"],"defaultCurrency":"USD","defaultLocale":null,"disableQtipWindowScroll":false,"doNotStoreCookiesRequireProvidedAnonId":false,"doNotTrackPingRequestsForActions":false,"enableCorsRestrictedOrigins":false,"enableMessageRotation":false,"enableRememberMeUserIds":false,"enableTemplateEsc":false,"hideContentSections":false,"hideContentSectionsMillis":2500,"hidePagesForRedirect":false,"hidePagesForRedirectMillis":1000,"identityAttributes":["profileId"],"lastModified":1607725715281,"preventSensitiveDataCapture":false,"rememberMeUserIdsMillis":63072000000,"sendErrorEvents":true,"showPoweredBy":false,"siteConfigExecBeforePageReady":false,"spaRouteChangeTimeout":500,"trackAnonymousVisitors":true,"trackContextualRelatedItems":false,"trackSubdomainAsCompany":false,"trackUnknownPagesByTitle":false,"treatHashChangeAsPageLoad":false}       );
    } catch (e) {
          console.error(e);
    }
})();
...
Evergage.init({
    ...
    const getCategoryFromDataLayer = () => {
        const objPageType = findInDataLayer("page");
        if (objPageType.page.department) {
            return objPageType.page.department;
        }
        return false;
    };

    const getSubcategoryFromDataLayer = () => {
        const objPageType = findInDataLayer("page");
        if (objPageType.page.category) {
            return objPageType.page.category;
        }
        return false;
    };

    const getPageTypeFromDataLayer = () => {
        const objPageType = findInDataLayer("page");
        if (objPageType.page.type) {
            return objPageType.page.type;
        }
        return false;
    };

    const getPageTemplateFromDataLayer = () => {
        const objPageTemplate = findInDataLayer("page");
        if (objPageTemplate && objPageTemplate.page.template) {
            return objPageTemplate.page.template;
        }
        return false;
    };

    const getLocale = () => {
        const objLanguage = findInDataLayer("pageLanguage");
        return objLanguage.pageLanguage;
    };
    ...
    const getCategoryFromUrl = () => {
        ...
        const strPath = url.pathname;
        let strCategory = strPath.substring(7);
        const slashCount = strCategory.split("/").length - 1;
        if (slashCount > 2) {
            const slashPosition =
                getPosition("/", strCategory, slashCount - 2) - 1;
            strCategory = strCategory.substring(slashPosition);
        }
        return strCategory.replaceAll("/", " ").trim().replaceAll("-", "");
    };

    const regExLoginOnly = /\/login\/(?!\S)/g;
    const regExLoginRegister = /\/login\/\?action=register/g;

    const config = {
        global: {
            locale: getLocale(),
            onActionEvent: (actionEvent) => {
                const email = Evergage.util.getValueFromNestedObject(
            ...
            contentZones: [
                {
                    name: "global_menuAsset",
                    selector: ".nav-content-asset-col.d-flex",
                },
            ],
            listeners: [
                Evergage.listener(
                    "submit",
                    ".footer-container .newsletterSubscribe",
                    () => {
                        const email = Evergage.cashDom(
                            "#footer-email-signup"
                        ).val();
                        if (email) {
                            Evergage.sendEvent({
                ...
                Evergage.listener(
                    "submit",
                    ".modal-body .newsletterSubscribe",
                    () => {
                        const email = Evergage.cashDom(
                            "#email-signup-field"
                        ).val();
                        if (email) {
                            Evergage.sendEvent({
                                action: "Email Sign Up - Modal",
                                user: { attributes: { emailAddress: email } },
                            });
                        }
                    }
                ),
                Evergage.listener("click", ".checkout-btn", () => {
                    Evergage.sendEvent({
                        action: "Begin Checkout",
                        user: { attributes: { hasBegunCheckout: true } },
                    });
                }),
                Evergage.listener("load", "#main-content", () => {
                    if (window.dataLayer[0]) {
                        if (
                            window.dataLayer[0].visitor &&
                            window.dataLayer[0].visitor.state &&
                            window.dataLayer[0].visitor.state === "recognized"
                        ) {
                            const HEM = window.dataLayer[0].visitor.emailHash
                                ? window.dataLayer[0].visitor.emailHash
                                : null;
                            const customerID = window.dataLayer[0].visitor
                                .customerID
                                ? window.dataLayer[0].visitor.customerID
                                : null;
                            const objUrl = new URL(window.location.href);
                            const customerUTM = `cid: ${objUrl.searchParams.get(
                                "cid"
                            )}, utm_source: ${objUrl.searchParams.get(
                                "utm_source"
                            )}, utm_medium: ${objUrl.searchParams.get(
                                "utm_medium"
                            )}, utm_campaign: ${objUrl.searchParams.get(
                                "utm_campaign"
                            )}, utm_content: ${objUrl.searchParams.get(
                                "utm_content"
                            )}`;
                            const customerHashedEmail = objUrl.searchParams.get(
                                "e"
                            );
                            Evergage.sendEvent({
                                action: "UTM",
                                user: { attributes: { UTM: customerUTM } },
                            });
                            if (HEM) {
                                Evergage.sendEvent({
                                    action: "HEM",
                                    user: { attributes: { hashedEmail: HEM } },
                                });
                            }
                            if (customerID) {
                                Evergage.sendEvent({
                            ...
                            if (customerHashedEmail) {
                                Evergage.sendEvent({
                                    action: "Customer Hashed Email",
                                    user: {
                                        attributes: {
                ...
                Evergage.listener("click", ".remove-product", (e) => {
                    const lineItem = Evergage.cashDom(e.target).closest(
                        ".product-line-item"
                    );
                    const id = Evergage.cashDom(lineItem)
                        .find(".item-number .attribute-value")
                        .text();
                    const price = Evergage.cashDom(lineItem)
                        .find(".price .value")
                        .attr("content");
                    const quantity = Evergage.cashDom(lineItem)
                        .find(".line-item-quantity .quantity")
                        .val();
                    console.log("REMOVE PRODUCT", id, price, quantity);
                    Evergage.sendEvent({
                        itemAction: Evergage.ItemAction.RemoveFromCart,
                        user: { attributes: { hasRemovedItem: true } },
                        cart: {
                            singleLine: {
                                Product: {
                                    _id: id,
        ...
        pageTypes: [
            {
                name: "home",
                isMatch: () => {
                    const currentPageType = getPageTypeFromDataLayer();
                    return currentPageType === "home";
                },
                contentZones: [
                    {
                        name: "home_slotPromo1",
                        selector: ".homepage-slot:nth-child(1)",
                    },
                    {
                        name: "home_slotPromo2",
                        selector: ".homepage-slot:nth-child(2)",
                    },
                    {
                        name: "home_slotPromo3",
                        selector: ".homepage-slot:nth-child(3)",
                    },
                    {
                        name: "home_slot1",
                        selector: ".homepage-slot:nth-child(4)",
                    },
                    {
                        name: "home_slot2",
                        selector: ".homepage-slot:nth-child(5)",
                    },
                    {
                        name: "home_slot3",
                        selector: ".homepage-slot:nth-child(6)",
                    },
                    {
                        name: "home_slot4",
                        selector: ".homepage-slot:nth-child(7)",
                    },
                ],
            },
            {
                name: "login",
                isMatch: () => regExLoginOnly.test(window.location.href),
                listeners: [
                    ...
                    Evergage.listener("submit", ".trackorder", () => {
                        const email = Evergage.cashDom(
            ...
            {
                name: "createAccount",
                isMatch: () => regExLoginRegister.test(window.location.href),
                listeners: [
                    Evergage.listener("submit", ".account-registration", () => {
                        ...
                        const phone = Evergage.cashDom(
                            "#registration-form-phone"
                        ).val();
                        const email = Evergage.cashDom(
                            "#registration-form-email"
                        ).val();
                        const optIn = Evergage.cashDom(
                            "#add-to-email-list"
                        ).val();
                        if (email) {
                            Evergage.sendEvent({
            ...
            },
            {
                ...
                isMatch: () => /\/communication/.test(window.location.href),
                listeners: [
                    Evergage.listener("submit", "#MCPreferencesForm", () => {
                        const email = Evergage.cashDom("#EmailAddress").val();
                        const interests = [
                            {
                                name: "Promotions",
                                value: Evergage.cashDom(
                                    "#dwfrm_mcpreferences_newsletterPreferences_Pref__Promotions"
                                )
                                    ? Evergage.cashDom(
                                          "#dwfrm_mcpreferences_newsletterPreferences_Pref__Promotions"
                                      ).val()
                            ...
                            },
                            {
                                name: "Gifting",
                                value: Evergage.cashDom(
                                    "#dwfrm_mcpreferences_newsletterPreferences_Pref__Gifting"
                                )
                                    ? Evergage.cashDom(
                                          "#dwfrm_mcpreferences_newsletterPreferences_Pref__Gifting"
                                      ).val()
                            ...
                            },
                            {
                                name: "Smartwatches",
                                value: Evergage.cashDom(
                                    "#dwfrm_mcpreferences_newsletterPreferences_Pref__Smartwatches"
                                )
                                    ? Evergage.cashDom(
                                          "#dwfrm_mcpreferences_newsletterPreferences_Pref__Smartwatches"
                                      ).val()
                            ...
                            },
                            {
                                name: "Accessories",
                                value: Evergage.cashDom(
                                    "#dwfrm_mcpreferences_newsletterPreferences_Pref__Accessories"
                                )
                                    ? Evergage.cashDom(
                                          "#dwfrm_mcpreferences_newsletterPreferences_Pref__Accessories"
                                      ).val()
                        ...
                        const zipCode = Evergage.cashDom("#postalCode").val();
                        const birthMonth = Evergage.cashDom(
                            "#mcpreferences_month"
                        ).val();
                        const birthYear = Evergage.cashDom(
                            "#mcpreferences_year"
                        ).val();
                        const gender = Evergage.cashDom(
                            "#mcpreferences_gender"
                        ).val();
                        if (email) {
                            Evergage.sendEvent({
                                action: "Email - Communications",
                                user: { attributes: { emailAddress: email } },
                            });
                        }
                        if (zipCode) {
                            Evergage.sendEvent({
                                action: "Zipcode - Communications",
                                user: { attributes: { zipcode: zipCode } },
                            });
                        }
                        if (birthMonth) {
                            Evergage.sendEvent({
                                action: "Birth Month - Communications",
                                user: {
                                    attributes: { birthMonth: birthMonth },
                                },
                            });
                        }
                        if (birthYear) {
                            Evergage.sendEvent({
                                action: "Birth Year - Communications",
                                user: { attributes: { birthYear: birthYear } },
                            });
                        }
                        if (interests[0].value !== null) {
                            Evergage.sendEvent({
                                action: "Interests - Communications",
                                user: { attributes: { interests: interests } },
                            });
                        }
                        if (gender) {
                            Evergage.sendEvent({
                                action: "Gender - Communications",
            ...
            },
            {
                ...
                contentZones: [
                    {
                        name: "cart_needHelp",
                        selector: ".cart-help-content-wrapper",
                    },
                    {
                        name: "cart_recommendations",
                        selector: ".recommendations:first-child",
                    },
                    {
                        name: "cart_recentlyViewed",
                        selector: ".recommendations:last-child",
                    },
                ],
            },
            {
                name: "checkout",
                isMatch: () => /\/Checkout-Begin/.test(window.location.href),
                contentZones: [
                    { name: "checkout_needHelp", selector: ".help-text" },
                ],
                listeners: [
                    Evergage.listener("submit", ".shipping-form", () => {
                        const email = Evergage.cashDom("#shippingEmail").val();
                        if (email) {
                            Evergage.sendEvent({
                                action: "Email - Checkout",
                                user: { attributes: { emailAddress: email } },
                            });
                        }
                    }),
                    Evergage.listener("click", ".submit-shipping", () => {
                        ...
                        const address1 = Evergage.cashDom(
                            "#shippingAddressOne"
                        ).val();
                        const address2 = Evergage.cashDom(
                            "#shippingAddressTwo"
                        ).val();
                        const state = Evergage.cashDom("#shippingState").val();
                        const city = Evergage.cashDom(
                            "#shippingAddressCity"
                        ).val();
                        const zip = Evergage.cashDom("#shippingZipCode").val();
                        const phone = Evergage.cashDom(
                            "#shippingPhoneNumber"
                        ).val();
                        if (email) {
                            Evergage.sendEvent({
                                action: "Complete Shipping Checkout",
                                user: {
                                    attributes: {
                                        emailAddress: email,
                                        optIn: optIn,
                    ...
                    Evergage.listener("click", ".submit-payment", () => {
                        Evergage.sendEvent({
                            action: "Complete Payment Checkout",
                            user: { attributes: { hasSubmitedPayment: true } },
                        });
                    }),
                ],
            },
            {
                ...
                order: {
                    Product: {
                        orderId: Evergage.resolvers.fromSelector(
                            ".order-number"
                        ),
                        lineItems: {
                            _id: () => {
                                var data = Evergage.resolvers.fromSelectorMultiple(
                                    ".item-number .attribute-value"
                                )();
                                console.log("ID", data);
                                return data;
                            },
                            price: () => {
                                var data = Evergage.resolvers.fromSelectorAttributeMultiple(
                                    ".line-item-product-price .sales .value",
                                    "content"
                                )();
                            ...
                            quantity: () => {
                                var data = Evergage.resolvers.fromSelectorMultiple(
                                    ".line-item-quantity .qty-card-quantity-count"
                                )();
            ...
            },
            {
                ...
                isMatch: () => {
                    const currentPageType = getPageTypeFromDataLayer();
                    return currentPageType === "pdp";
                },
                catalog: {
                    Product: {
                        _id: () => {
                            return window.dataLayer[1].ecommerce
                                ? window.dataLayer[1].ecommerce.detail
                                      .products[0].id
                                : null;
                        },
                        name: () => {
                            return window.dataLayer[1].ecommerce &&
                                window.dataLayer[1].ecommerce.detail.products[0]
                                ? window.dataLayer[1].ecommerce.detail
                                      .products[0].name
                                : null;
                        },
                        ...
                        imageURL: Evergage.resolvers.fromSelectorAttribute(
                            ".primary-images .modal .photo:first-child img",
                            "src"
                        ),
                        inventoryCount: () => {
                            return window.dataLayer[1].ecommerce
                        ...
                        currencyCode: () => {
                            return window.dataLayer[1].ecommerce
                                ? window.dataLayer[1].ecommerce.currencyCode
                                : null;
                        },
                        price: () => {
                            return window.dataLayer[1].ecommerce
                ...
                contentZones: [
                    {
                        name: "product_shippingMessaging",
                        selector: ".detail-panel-group",
                    }, //TODO Not enough details
                    {
                        name: "product_productReco",
                        selector: ".product-detail .rec-product-tile-caro",
                    },
                    {
                        name: "product_recentlyViewed",
                        selector:
                            "#main-content > div[id^='cq_recomm_slot'] .rec-product-tile-caro",
                    },
                    {
                        name: "product_contentAssetId",
                        selector: ".custom-pdp-content .content-asset",
                    },
                ],
                listeners: [
                    Evergage.listener("click", ".add-to-cart", () => {
                        ...
                        const lineItem =
                            Evergage.util.buildLineItemFromPageState(
                        ...
                        lineItem.color = {
                            color: Evergage.cashDom(
                                "#colorCollapse .selected"
            ...
            },
            {
                name: "category",
                isMatch: () => {
                    const currentTemplate = getPageTemplateFromDataLayer();
                    const currentPageType = getPageTypeFromDataLayer();
                    if (
                        currentTemplate === "catLanding" &&
                        currentPageType === "department"
                    ) {
                        return true;
                    }
                    if (
                        currentTemplate === "categoryproducthits" &&
                ...
                catalog: {
                    Category: {
                        _id: getSubcategoryFromDataLayer,
                    },
                },
                contentZones: [
                    {
                        name: "category_slot1",
                        selector: ".catlanding-slot:nth-child(1)",
                    },
                    {
                        name: "category_slot2",
                        selector: ".catlanding-slot:nth-child(2)",
                    },
                    {
                        name: "category_slotPromo1",
                        selector: ".catlanding-slot:nth-child(3)",
                    },
                    {
                        name: "category_slotPromo2",
                        selector: ".catlanding-slot:nth-child(4)",
                    },
                    {
                        name: "category_slotPromo3",
                        selector: ".catlanding-slot:nth-child(5)",
                    },
                    {
                        name: "category_slotPromo4",
                        selector: ".catlanding-slot:nth-child(6)",
                    },
                ],
                listeners: [],
            },
            {
                ...
                catalog: {
                    Category: {
                        _id: getCategoryFromDataLayer,
                    },
                },
                contentZones: [
                    {
                        name: window.location.pathname === '/en-us/womens/' || window.location.pathname === '/en-us/mens/' ? window.location.pathname === '/en-us/womens/' ? "department_catBanner_Womens" : "department_catBanner_Mens" : "department_catBanner",
                        selector: ".slot-grid-header",
                    },
                    {
                        name: "department_gridContentAssetId",
                        selector: ".product-grid .content-asset",
                    },
                ],
            },
        ],
